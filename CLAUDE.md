...
The homepage build-log panel merges two sources (see `assets/js/activity.js`):

- GitHub public events API (public repos, live, includes commit messages)
- `assets/data/activity.json` — private-repo activity generated by `node scripts/update-activity.mjs`, which scans local clones in `~/Projects` and `~/Documents/GitHub`. Names and commit counts only — never commit messages. Repos can be excluded via `EXCLUDE` in the script.

Regenerate the JSON before any push to main (it's step 3 of `/blog-publish`). Stale data just shows honest relative dates; it fails soft.
